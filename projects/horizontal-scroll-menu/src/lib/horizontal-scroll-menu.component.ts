...
@Component({
  ...
  template: `
    <nav class="d-flex" *ngIf="items?.length" [ngClass]="background">
      <div class="control d-flex align-items-center justify-content-center mousePointer" (click)="scrollLeft()">
      ...
      <div class="control d-flex justify-content-center align-items-center mousePointer" (click)="scrollRight()">
...
export class HorizontalScrollMenuComponent implements OnInit, OnChanges {
  ...
  generateItems(): any[] {
    const items = [];
    for (let i = 1; i < 50; i++) {
      items.push({ title: `Item${i}`, link: 'https://google.com' });
    }
